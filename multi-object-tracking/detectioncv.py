...
def image2tensor(image):
    ...
    # Write code here
    image = np.array(image).astype(np.float32)  # convert frame to float
    image = resize(image,(300,300,3))  # resize image to 300x300
    image = image[:,:,::-1] .astype(np.float32)  # convert RGB to BGR
    image = image-IMAGENET_MEAN.astype(np.float32)  # center with respect to imagenet means
...
def extract_detections(frame, min_confidence=0.6, labels=None):
    ...
    results = bbox_util.detection_out(inputnp,confidence_threshold=min_confidence)[0]
    ...
    if labels is not None:
        ...
        result_labels = result_labels.astype(np.int32)
        indices = [
            index
    ...
    if max(np.array(results).shape)>5:
        results=np.array(results)[:,[0, 2, 3, 4, 5]]
        results[:, [1, 3]] *= frame.shape[1]
        results[:, [2, 4]] *= frame.shape[0]
